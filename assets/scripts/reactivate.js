...
$('.btn-send').on('click', function () {
  ...
  if (bot_trap.length == 0) {
    ...
    var ajax_url = 'https://baseball-star.com/backend/handlers/reactivate.php';
    ...
    if (ajax_url.length > 0) {
      ...
      if (verify_code.length >= 30 && email.length > 0 && password.length > 0) {
          ...
          $.ajax({
            ...
            success: function(response) {
              ...
              if (response === 'error') {
                ...
                // Error message
                $('.messages').html('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>We couldn\'t reactivate your account. Double check the password and email you entered or contact us.</div>');
    ...
    // ajax_url is not greater than 0.
      ...
      // Error message
      $('.messages').html('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>There was an issue reactivating your account. Please contact us or try again.</div>');
  ...
  // Bot trap input was filled out.
    ...
    // Error message
    $('.messages').html('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>An invalid action was performed during account reactivation. Please contact us or try again.</div>');
